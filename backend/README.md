...
- `/probe` - a simple GET endpoint to check that the server is live and responding
...
The actual server is run and distributed as a Docker image. The Dockerfile in this folder is responsible for:
...
the image for the server can also be built from the Dockerfile in this folder:
