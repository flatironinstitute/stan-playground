...
const mockModule = (p: Partial<ModuleSettings>) => {
    ...
    const module = {
        // malloc returns unique number
        _malloc: vi.fn(() => (malloc_base++ * 8) as unknown as ptr),
        _free: vi.fn(),
        _tinystan_create_model: vi.fn(() => (malloc_base++ * 8) as unknown as model_ptr),
        ...
        _tinystan_get_error_message: vi.fn((ptr) => `Error at address ${ptr}` as unknown as cstr),
...
expect.extend({
    toHaveNoMemoryLeaks: (module: MockedModule, _) => {
        // checks that each malloc call is matched by a free call
        const freedAddresses = module._free.mock.calls.map((args) => args[0]).filter((x) => x !== 0);
        for (const { value } of module._malloc.mock.results) {
            if (!freedAddresses.includes(value)) {
                return {
                    pass: false,
                    message: () => `Memory leak detected! Address ${value} was never freed.`,
                };
            }
        }

        const uniqueFreedAddresses = new Set(freedAddresses.filter((x: any) => x !== 0));
        if (uniqueFreedAddresses.size !== freedAddresses.length) {
            return {
                pass: false,
                message: () => `Memory leak detected! Some addresses were freed multiple times: ${freedAddresses}`,

            };
        }

        // checks that the Stan Model is eventually destroyed
        const destroyedModels = module._tinystan_destroy_model.mock.calls.map((args: any) => args[0]);
        for (const { value } of module._tinystan_create_model.mock.results) {
            if (!destroyedModels.includes(value)) {
                return {
                    pass: false,
                    message: () => `Model at address ${value} was never destroyed.`,
                };
            }
        }
