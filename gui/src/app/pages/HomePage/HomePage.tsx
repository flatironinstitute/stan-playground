...
const DataEditor: FunctionComponent<unknown> = () => {
  ...
  const dataSourceDesc = useMemo(() => {
    ...
    switch (data.meta.dataSource) {
      ...
      case DataSource.GENERATED_BY_STALE_PYTHON: {
        return "a prior version of data.py.";
      }
      case DataSource.GENERATED_BY_STALE_R: {
        return "a prior version of data.R.";
      }
      default:
  ...
  const dataMessageColor: ColorOptions = useMemo(() => {
    if (data.meta.dataSource) {
      if (dataSourcesNeedingWarning.includes(data.meta.dataSource)) {
        return "warning.main";
      }
    }
    return "info.main";
  ...
  const dataMessage: ToolbarItem[] = useMemo(() => {
    if (dataSourceDesc === undefined) {
      ...
      return [
        {
          type: "text",
          label: `Data is generated by ${dataSourceDesc}`,
          color: dataMessageColor,
        },
      ];
    }
  }, [dataMessageColor, dataSourceDesc]);
