...
const CompilationServerConnectionControl: FunctionComponent<CompilationServerConnectionControlProps> = () => {
    const [stanWasmServerUrl, setStanWasmServerUrl] = useState<string>(localStorage.getItem('stanWasmServerUrl') || publicUrl)
    ...
    const {handleOpen: openDialog, handleClose: closeDialog, visible: dialogVisible} = useModalWindow()

    const handleRetry = useCallback(() => {
        retryConnection()
    }, [retryConnection])

    const serverLabel = stanWasmServerUrl === publicUrl ? 'public' : stanWasmServerUrl === localUrl ? 'local' : 'custom'
    return (
        <span>
...
const ConfigureCompilationServerDialog: FunctionComponent<ConfigureCompilationServerDialogProps> = ({stanWasmServerUrl, setStanWasmServerUrl, isConnected, closeDialog, onRetry}) => {
    ...
    return (
        <div>
            ...
            <hr />
            <div>
                <input type="radio" id="public" name="server" value="public" checked={choice === 'public'} onChange={() => {
                ...
                <input type="radio" id="local" name="server" value="local" checked={choice === 'local'} onChange={() => {
                ...
                <input type="radio" id="custom" name="server" value="custom" checked={choice === 'custom'} onChange={() => setChoice('custom')} />
                <label htmlFor="custom">Custom server</label>
                <br />

                <input style={{width: 500}} disabled={choice !== 'custom'} type="text" value={stanWasmServerUrl} onChange={e => setStanWasmServerUrl(e.target.value)} />
                <br />
                <hr />
                {
                    choice === 'local' && (
                ...
                }
                {
                    choice === 'public' && (
                        <div>
                            <p>
                                The public server is provided for convenience, but may not be as reliable as a local server,
                                depending on the current load and availability.
                            </p>
                        </div>
                    )
                }
